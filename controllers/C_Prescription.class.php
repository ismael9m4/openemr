...
//     }
    ...
    function multiprint_header(&$pdf, $p)
    {
        ...
        $res = sqlQuery("SELECT concat('<b>','Servicio Médico Vital','</b>\n',f.street,'\n',f.city,', ',f.state,' ','\nTel:',f.phone) addr FROM users JOIN facility AS f ON f.name = users.facility where users.id ='" .
            add_escape_custom($p->provider->id) . "'");
        $pdf->ezText($res['addr'] ?? '', 9);
        $my_y = $pdf->y;
        $pdf->ezNewPage();
        $pdf->ezText('<b>' . $p->provider->get_name_display() . '</b>', 9);
    ...
    //     Administration->Globals->Rx
        if ($GLOBALS['rx_enable_DEA']) {
            if ($this->is_faxing || $GLOBALS['rx_show_DEA']) {
                $pdf->ezText('<b>' . xl('DEA') . ':</b>' . $p->provider->federal_drug_id, 7);
            } else {
                //$pdf->ezText('<b>' . xl('DEA') . ':</b> _____$p->provider->id___________________', 7);
		$pdf->ezText('<b>' . xl('M.P.') . ':</b>' . $p->provider->npi, 7);
            }
        }

        if ($GLOBALS['rx_enable_NPI']) {
            if ($this->is_faxing || $GLOBALS['rx_show_NPI']) {
                    $pdf->ezText('<b>' . xl('NPI') . ':</b>' . $p->provider->npi, 5);
            } else {
                $pdf->ezText('<b>' . xl('NPI') . ':</b> _________________________', 5);
            }
        }
		...
		//Codigo Agregado para calcular la Edad a partir de la date_of_birth
        ...
        $pdf->ezText('');
        $pdf->line($pdf->ez['leftMargin'], $pdf->y, $pdf->ez['pageWidth'] - $pdf->ez['rightMargin'], $pdf->y);
        $pdf->ezText('<b>' . xl('DNI Paciente:') . '</b>', 6);
    ...
    function multiprint_action($id = "")
    {
        ...
        $modedFileName = "Receta_{$pFName}_{$p->patient->id}.pdf";
