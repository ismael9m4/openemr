...
function duplicateRow(e) {
    ...
    changeIds('user');
    changeIds('count');
    changeIds('cara'); // Nuevo campo select
    changeIds('diente'); // Nuevo campo
...
function removeVal(rowid) {
    ...
    document.getElementById("cara_" + rowid1[1]).selectedIndex = 0; // Limpiar select cara
    document.getElementById("diente_" + rowid1[1]).value = ''; // Limpiar campo diente
...
function changeIds(class_val) {
    ...
    for (let i = 0; i < elem.length; i++) {
        ...
        if (class_val === 'cara') {
            elem[i].selectedIndex = 0; // Limpiar el select
        }
    }
}
