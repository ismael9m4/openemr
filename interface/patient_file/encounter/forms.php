...
if (
    ...
    ($formdir == 'newpatient' && $_SESSION['authUserID'] == $providerIDres) ||
    $_SESSION['authUserID'] == 37
) {
    ...
    echo xlt('Edit') . "</a>";
} elseif ($formdir != 'newpatient' && $_SESSION['authUser'] == $iter['user']) {
    echo "<a class='btn btn-text btn-sm form-edit-button btn-edit' " .
        "id='form-edit-button-" . attr($formdir) . "-" . attr($iter['id']) . "' " .
        "href='#' " .
        "title='" . xla('Edit this form') . "' " .
    ...
    echo xlt('Edit') . "</a>";
        ...
        if (
    ($_SESSION['authUser'] == $iter['user']) ||
    ($_SESSION['authUserID'] == 37)
) {
    if (
        ($esign->isButtonViewable() && $is_group == 0 && $authPostCalendarCategoryWrite) ||
        ($esign->isButtonViewable() && $is_group && AclMain::aclCheckCore("groups", "glog", false, 'write') && $authPostCalendarCategoryWrite)
    ) {
